...
from house.entity.artifact_entity import DataIngestionArtifact , DataValidationArtifact, DataTransformationArtifact,ModelTrainerArtifact,ModelEvaluationArtifact
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            ...
            logging.info(f"Converting the target columns into numpy array")
            train_target_arr = np.array(train_dataframe[target_column_name])
            
            test_target_arr= np.array(test_dataframe[target_column_name])
            
            logging.info(f"Conversion complete target column into numpy array.")
            
            
            ##dropping the target columns from the dataframe
            
            logging.info(f"Dropping target columns from the dataframe")
            train_dataframe.drop(target_column_name,axis=1,inplace=True)
            test_dataframe.drop(target_column_name,axis=1,inplace=True)
            logging.info(f"Dropping target columns from the dataframe completed")
            
            model=self.get_best_model()
            
            if model is None:
                logging.info("Not found any existing model. Hence accept the trained model")
                model_evaluation_artifact= ModelEvaluationArtifact(is_model_accepted=True,
                                                                   evaluated_model_path=trained_model_file_path)
                self.update_evaluation_report(model_evaluation_artifact)
                logging.info(f"Model accepted, Model evel artifact{model_evaluation_artifact} created ")
            ...
            logging.info(f"Model evaluation completed Model evaluation artifact:  {metric_info_artifact}")
            
            if metric_info_artifact is None:
                response = ModelEvaluationArtifact(is_model_accepted=True,evaluated_model_path=trained_model_file_path)
