...
class DataIngestion:
    def __init__(self,data_ingestion_config=Configuration()):
        try:
            logging.info(f"{'='*20}Data Ingestion log started.{'='*20}")
            self.data_ingestion_config=data_ingestion_config.get_data_ingestion_config()
        except Exception as e:
            raise HouseException(e,sys) from e
        
    def download_house_data(self)->str:
        try:
            ...
            ## DOWNLOAD THE FILE FROM URL AND SAVE IN DIR 
    ...
    def split_the_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            raw_data_dir = self.data_ingestion_config.raw_data_dir
            file_name = os.listdir(raw_data_dir)[0] ## this is will return the list 
            ...
            if strat_test_set is not None:
                ...
                logging.info(f"Exporting train dataset to file : [{test_file_path}]")
    ...
    def initiate_data_ingestion(self)-> DataIngestionArtifact:
        try:
            tgz_file_path=self.download_house_data()
            self.extract_tgz_file(tgz_file_path=tgz_file_path)
            return self.split_the_data_as_train_test()

        except Exception as e:
            raise HouseException(e,sys) from e
        

    def __del__(self): ## whenever it is distract 
