...
GridSearchedBestModel = namedtuple("GridSearchedBestModel",
                                   ["model_serial_number","best_model",
...
MetricInfoArtifact = namedtuple("MetricInfoArtifact",["model_name","model_object","train_rmse",
...
def evaluate_regression_model(model_list:list,x_train:np.ndarray,y_train:np.ndarray,x_test: np.ndarray,y_test:np.ndarray,base_accuracy:float=0.6)-> MetricInfoArtifact:
    ...
    Params:
        ...
        y_test (np.ndarray): testing dataset input feature

    Returns:
        it returned a named tuple
        MetricInfoArtifact = namedtuple("MetricInfo",["model_name","model_object","train_rmse",
                                                      "test_rmse","train_accuracy","test_accuracy",
                                                      "model_accuracy","index_number"])
    """
    ...
    try:
        ...
        for model in model_list:
            model_name=str(model) ## getting the model name based on the model object
            logging.info(f"{'>>'*30}Started evaluating model: [{type(model).__name__}]{'<<'*30}")
            
            ## getting the prediction of training and testing dataset
            y_train_pred=model.predict(x_train)
            y_test_pred=model.predict(x_test)
            
            ## calculating the r squared score on training and testing dataset
            ...
            ## calculating the harmonic mean of train_accuracy and test_accuracy
            ...
            logging.info(f"Diff test and train accuracy: [{diff_test_train_acc}]")
            ...
            ## we will accept that model ad accepted model
...
def get_sample_mode_config_yaml_file(export_dir:str):
    try:
        model_config={
            GRID_SEARCH_KEY:{
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref:object,property_data:dict):
        try:
            if not isinstance(property_data,dict):
                raise Exception("property_data parameter should be a dictionary")
            print(property_data)
    ...
    def get_initialized_model_list(self)-> List[InitializedModelDetail]:
        ...
        Returns:
            List[InitializedModelDetail]: _description_
        """
    ...
    def initiate_best_parameter_search_for_initialized_model(self,initialized_model:InitializedModelDetail,
        ...
        input_feature:your all input features
        output_feature:Your target/dependent features
        Returns: Function will return a GridSearchOperation
    ...
    def get_model_detail(model_details: List[InitializedModelDetail],
        ...
        """
        This function return ModelDetails
        """
        ...
        try:
            ...
            for grid_searched_best_model in grid_searched_best_model_list:
                if base_accuracy<grid_searched_best_model.best_score:
                    logging.info(f"Accept model found:{grid_searched_best_model}")
